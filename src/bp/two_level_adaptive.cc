...
struct history_register_table {  // use HHRT for now (though performance might
                                 // be ... suboptimal)
  ...
  void init_hashtable(uns hhrt_size, uns hrt_entry_size) {
    init_hash_table(&HHRT, "hash_history_register_table", hhrt_size,
                    hrt_entry_size);
    Flag init_hash_history_register_table;  // NEW: must initialize all entries
                                            // of set_entries_occupied_table to
                                            // 0
    int* set_table_entry;                   // (This is to avoid junk values)

    for(int i = 0; i < hhrt; i++) {
      set_table_entry = hash_table_access_create(
        &HHRT, i, &init_hash_history_register_table);
      *set_table_entry = 0u;
    }
  }
  ...
  int get_HR_content(Addr branch_addr) {
    ...
    uns history_bits* = hash_table_access_create(
      &HHRT, branch_addr,
      &HR_content_flag);  // how to handle collisions? Do we need to?
    return *history_bits;
  }

  void update_HR_content(Addr branch_addr, uns branch_outcome) {
    Flag HR_content_flag;
    uns  current_HR_content = hash_table_access(&HHRT, branch_addr,
                                                &HR_content_flag);
...
struct branch_history_pattern_table {
  uns8 PT = uns8[pow(2, HRT_entry_size)]
    ...
    uns8
    get_prediction(int hr_content) {
    return PT[hr_content];
  }

  void change_SC(uns hr_content, uns current_Sc, int automata_to_use) {
    uns8 last_outcome = hr_content[0];  // yes, this is horribly incorrect,
                                        // change to use proper bit operations
    uns8 SC_plus_one = 0u;

    switch(automata_to_use) {
      case 0:
        SC_plus_one = LT_Last_Time(last_outcome);
        break;
      default:
        SC_plus_one = LT_Last_Time(last_outcome);
        break;
...
void bp_two_level_adaptive_init() {
  ...
  uns HHRT_size = 512;  // We'll start with the most common sizes used in the
                        // paper's tests
  uns HRT_entry_size = 12;

  // for(auto& two_level_adaptive_state : two_level_adaptive_state_all_cores) {
  // // might not need this
  HRT.init_hashtable(
    HHRT_size, HRT_entry_size);  // this might not work, change accordingly
  //}
}
...
void bp_two_level_adaptive_update(Op* op) {
  if(op->table_info->cf_type != CF_CBR) {  // If op isn't conditional branch,
                                           // don't interact with gshare.
    return;
  }
